...
function HomePage({ pin, category, data }: { pin: string; category: Category; data: any }) {
  ...
  return <div className="space-y-4 pb-2">
    ...
    <Pomodoro pin={pin} />
    <Card className="border-dashed border-emerald-500/45 bg-emerald-50/45 dark:bg-emerald-950/20"><CardContent className="flex min-h-28 items-center gap-4 p-4"><div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl border border-dashed border-emerald-500/50 bg-background/60"><Sprout className="h-6 w-6 text-emerald-600" /></div><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><p className="font-semibold">モンスター育成</p><Badge variant="outline" className="border-emerald-500/40 text-[10px] text-emerald-700 dark:text-emerald-300">後日実装</Badge></div><p className="mt-1 text-xs leading-5 text-muted-foreground">学習時間・習得単語・連続学習に連動する育成機能のための専用スペースです。</p></div></CardContent></Card>
