...
function CalendarGardenPager({ sessions, events }: { sessions: { createdAt: Date }[]; events: { eventDate: string; title: string }[] }) {
  const [page, setPage] = useState<"calendar" | "garden">("calendar");
  const touchStart = useRef<number | null>(null);
  const onSwipeEnd = (clientX: number) => {
    if (touchStart.current === null) return;
    const difference = clientX - touchStart.current;
    if (Math.abs(difference) > 45) setPage(difference < 0 ? "garden" : "calendar");
    touchStart.current = null;
  };
  return <div className="space-y-2"><div className="overflow-hidden rounded-2xl"><motion.div className="flex w-[200%]" animate={{ x: page === "calendar" ? "0%" : "-50%" }} transition={{ duration: 0.26, ease: [0.23, 1, 0.32, 1] }} onTouchStart={event => { touchStart.current = event.touches[0]?.clientX ?? null; }} onTouchEnd={event => onSwipeEnd(event.changedTouches[0]?.clientX ?? 0)}><div className="w-1/2 pr-1"><Card><CardContent className="p-4"><CalendarGrid sessions={sessions} events={events} /></CardContent></Card></div><div className="w-1/2 pl-1"><Card className="h-full min-h-72 border-dashed border-emerald-500/45 bg-emerald-50/45 dark:bg-emerald-950/20"><CardContent className="flex h-full min-h-72 flex-col items-center justify-center p-6 text-center"><div className="flex h-16 w-16 items-center justify-center rounded-2xl border border-dashed border-emerald-500/50 bg-background/60"><Sprout className="h-7 w-7 text-emerald-600" /></div><Badge variant="outline" className="mt-4 border-emerald-500/40 text-emerald-700 dark:text-emerald-300">後日実装</Badge><p className="mt-2 font-semibold">モンスター育成エリア</p><p className="mt-1 max-w-xs text-xs leading-5 text-muted-foreground">学習時間・習得単語・連続学習と連動する育成機能のための専用スペースです。</p></CardContent></Card></div></motion.div></div><div className="flex items-center justify-center gap-2"><button type="button" aria-label="カレンダーを表示" onClick={() => setPage("calendar")} className={cn("h-1.5 rounded-full transition-all", page === "calendar" ? "w-6 bg-primary" : "w-1.5 bg-muted-foreground/35")} /><button type="button" aria-label="モンスター育成エリアを表示" onClick={() => setPage("garden")} className={cn("h-1.5 rounded-full transition-all", page === "garden" ? "w-6 bg-primary" : "w-1.5 bg-muted-foreground/35")} /></div><p className="text-center text-[11px] text-muted-foreground">横にスワイプして {page === "calendar" ? "モンスター育成エリア" : "カレンダー"} を表示</p></div>;
}
...
function HomePage({ pin, category, data }: { pin: string; category: Category; data: any }) {
  ...
  return <div className="space-y-4 pb-2">
    ...
    <Pomodoro pin={pin} />
    <CalendarGardenPager sessions={data.sessions} events={[...data.events, ...(data.personalEvents ?? [])]} />
