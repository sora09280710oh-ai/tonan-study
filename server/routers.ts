...
export const appRouter = router({
  ...
  learning: router({
    ...
    gradeKanjiHandwriting: publicProcedure.input(z.object({ pin, expectedKanji: z.string().trim().min(1).max(20), imageDataUrl: z.string().min(100).max(3_000_000), strictness: z.enum(["standard", "strict"]).default("standard") })).mutation(({ input }) => gradeKanjiHandwriting(input.imageDataUrl, input.expectedKanji, input.strictness)),
  }),
  admin: router({
